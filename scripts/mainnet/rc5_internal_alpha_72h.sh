...
get_block_number() {
  local url="$1"
  rpc "$url" chain_getHeader '[]' | "$JQ_BIN" -r '.result.number // "0x0"' | xargs printf '%d\n' 2>/dev/null || echo 0
}
...
get_finalized_number() {
  ...
  rpc "$url" chain_getHeader "[\"$hash\"]" | "$JQ_BIN" -r '.result.number // "0x0"' | xargs printf '%d\n' 2>/dev/null || echo 0
}

get_peer_count() {
  local url="$1"
  rpc "$url" system_health '[]' | "$JQ_BIN" -r '.result.peers // 0' 2>/dev/null || echo 0
}
...
get_runtime_spec_version() {
  local url="$1"
  rpc "$url" state_getRuntimeVersion '[]' | "$JQ_BIN" -r '.result.specVersion // 0'
}
