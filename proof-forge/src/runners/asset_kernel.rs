...
pub async fn verify_claim(
    ...
    match invariant_search {
        Ok(output) => {
            ...
            evidence.insert("invariants_declared".to_string(), invariant_count.to_string());
    ...
    let monitor_search = Command::new("grep")
        .args(&["-r", "supply_monitor", "pallets/x3-kernel/", "--include=*.rs"])
        .current_dir(workspace)
        .output();
    ...
    let score = if test_passed {
        let base_score = 0.5;
        let has_invariants = evidence.get("invariants_declared")
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(0) > 0;
        let has_monitor = evidence.contains_key("supply_monitor");
        
        base_score + (if has_invariants { 0.25 } else { 0.0 }) + (if has_monitor { 0.25 } else { 0.0 })
    } else {
        0.0
    };
    ...
    let result = ProofResult {
        ...
        claim: "Asset kernel maintains canonical supply invariant across all operations".to_string(),
        status,
    ...
    if verbose {
        ...
        println!("    Passed: {} | Failed: {}", result.passed_checks.len(), result.failed_checks.len());
    }
