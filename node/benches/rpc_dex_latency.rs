...
impl MockDexRpcClient {
    ...
    async fn estimate_swap(&self, request: serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        let amount_in = request["amount_in"].as_str().unwrap_or("1000000").parse::<u128>().unwrap_or(1000000);
...
async fn benchmark_estimate_swap_latency(client: &MockDexRpcClient, iterations: usize) -> (f64, f64, f64, f64) {
...
async fn benchmark_execute_swap_latency(client: &MockDexRpcClient, iterations: usize) -> (f64, f64, f64, f64) {
    let mut latencies = Vec::with_capacity(iterations);
